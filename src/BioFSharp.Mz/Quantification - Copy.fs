...
//    open PeakDetection.Wavelet

//    type PeakToQuantify<'T> = {
//        ApexY : float 
//        ApexX : float
//        XData : float []
//        YData : float []
//        ParameterEstimates: 'T option
//        }
...
//        XData                   : float []
//        YData                   : float []
//        PeaksToQuantify         : seq<PeakToQuantify<'T>>
//        HasParameterEstimates   : bool
//        HasMultiplePeaks        : bool
//    }
...
//        ///
//        let methodOfMoments (p: PeakToQuantify<'a>) =
//            try
//            let modeY   = p.ApexY
...
//            | _ -> Option.None
//        ///
//        let estimatePeakIntegrity (p: PeakToQuantify<'a>) = 
//            let zipped = Array.zip p.XData p.YData 
//            let left  = zipped |> Array.exists (fun (x,y) -> x < p.ApexX && y > 0.2 * p.ApexY) 
//            let right = zipped |> Array.exists (fun (x,y) -> x > p.ApexX && y > 0.2 * p.ApexY)
//            left && right

//        ///
//        let estimateMoments (p: PeakToQuantify<'a> ) =
//            if estimatePeakIntegrity p then 
//                methodOfMoments p 
//            else 
//                let xData,yData = 
//                    Array.zip p.XData p.YData 
//                    |> Array.filter (fun (x,y) -> y > 0.3 * p.ApexY)
//                    |> Array.unzip
...
//                    Some (createEstimatedMoments p.ApexY caruanaEst.MeanX (caruanaEst.STD**2.) caruanaEst.STD skew tau)
//                | Option.None            -> 
//                    methodOfMoments p 
...
//        /// Return Option
//        let getPeakBy x (peaks:SignalEntity<'a> []) =
//            let isPartOfPeak x (p:SignalEntity<'a>)  = p.Start < x && p.End > x        
//            match Array.tryFind (isPartOfPeak x) peaks with
//            | Some p        ->  p
//            | Option.None   -> 
//                Array.minBy (fun p -> abs(0.5*(p.Start + p.End) - x)) peaks 
...
//            ///
//            let toSignalEntity (p:IdentifiedPeak ) :SignalEntity<'a option> = 
//                let peakToQuantify = {
//                    ApexY = p.Apex.YVal 
//                    ApexX = p.Apex.XVal
//                    XData = p.XData
//                    YData = p.YData
//                    ParameterEstimates = Option.None
//                    }

//                {
//                    Start                   = p.LeftEnd.XVal 
//                    End                     = p.RightEnd.XVal
//                    XData                   = p.XData
//                    YData                   = p.YData
//                    PeaksToQuantify         = [peakToQuantify]
//                    HasParameterEstimates   = false
//                    HasMultiplePeaks        = false
//                }

//        module Wavelet = 

//            ///
//            let toSignalEntity (p:PeakGroup) :SignalEntity<(EstimatedMoments)> = 
//                let peakToQuantify (g:Gaussian) : PeakToQuantify<EstimatedMoments> = 
//                    let x,y = g.Trace |> Array.unzip
//                    let tau = 
//                    let moments = createEstimatedMoments g.Amplitude g.XLoc g.Stdev (sqrt g.Stdev)
//                    {
//                    ApexY = g.XLoc 
//                    ApexX = g.Amplitude
//                    XData = x
//                    YData = y
//                    ParameterEstimates = (Option.Some moments)
//                    }

//                let x,y = p.Data |> Array.unzip 
//                {
//                    Start                   = p.Start 
//                    End                     = p.End
//                    XData                   = x
//                    YData                   = y
//                    PeaksToQuantify         = (p.Fits |> List.map peakToQuantify) 
//                    HasParameterEstimates   = true
//                    HasMultiplePeaks        = p.Convolved
//                }

//        ///
//        let quantifySinglePeak (getMoments:PeakToQuantify<'a> -> EstimatedMoments option) (p: PeakToQuantify<'a>) =
//            match getMoments p with 
//            | Some moments -> 
...
//                    createQuantifiedPeak  (Some finalFit.Model) finalFit.YPredicted finalFit.EstimatedParams finalFit.StandardErrorOfPrediction area p.ApexY
//                | Some finalFit, Option.None  | Option.None, Some finalFit -> 
//                    let area       = calcArea finalFit
//                    createQuantifiedPeak  (Some finalFit.Model) finalFit.YPredicted finalFit.EstimatedParams finalFit.StandardErrorOfPrediction area p.ApexY
//                | _                       ->
//                    let area = trapezEstAreaOf p.XData p.YData
//                    createQuantifiedPeak Option.None [||] [||] nan area p.ApexY
//            | Option.None   -> 
//                let area = trapezEstAreaOf p.XData p.YData
//                createQuantifiedPeak Option.None [||] [||] nan area p.ApexY         

//        ///
//        let quantifyConvolvedPeak (getMoments:SignalEntity<'a> -> (EstimatedMoments option) list) (p: SignalEntity<'a>) =
//            let moments = getMoments p |> List.choose id
//            match moments with 
//            | [] ->
//                let area = trapezEstAreaOf p.XData p.YData
//                createQuantifiedPeak Option.None [||] [||] nan area (p.YData |> Array.max)         
//            | _ -> 
//                let area = trapezEstAreaOf p.XData p.YData
//                createQuantifiedPeak Option.None [||] [||] nan area (p.YData |> Array.max)       
//            //match getMoments p with 
...
//        /////
//        //let quantifyBy (getMoments:PeakToQuantify<'a> -> EstimatedMoments option) (getMoments:SignalEntity<'a> -> EstimatedMoments option) x (peaks: SignalEntity<'a> [] ) =
//        //    let peakToQuantify = getPeakBy x peaks  
//        //    match peakToQuantify.HasMultiplePeaks with
//        //    | false  -> 
//        //        let p = peakToQuantify.PeaksToQuantify |> Seq.head
//        //        quantifyPeak getMoments p
//        //    | true-> 
//        //        let p = peakToQuantify.PeaksToQuantify |> Seq.head
//        //        quantifyPeak p
