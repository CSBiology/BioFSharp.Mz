...
module PeakList =
    ...
    let zip (mz:list<float>) (intensity:list<float>) : PeakList<_> = 
    ...
    /// Returns a new PeakList.
    let zipMzInt (mzIntensity:list<float*float>) : PeakList<_> = 
        List.map (fun (m,i) -> Peak(m,i)) mzIntensity
