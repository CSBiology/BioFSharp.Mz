...
module Quantification =
    ...
    //    let iterateTo step arr startIdx (test: 'a -> bool) =
    //        let rec loop  (arr: 'a []) currentIdx =
    //            if currentIdx <= 0 then None
    //            elif currentIdx >= arr.Length-1 then None
    //            else                                              
    //                match test arr.[currentIdx] with 
    //                | true -> Some currentIdx   
    //                | _               -> loop arr (currentIdx+step) 
    //        loop arr (startIdx+step) 
